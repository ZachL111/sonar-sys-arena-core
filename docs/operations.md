...
`sonar-sys-arena-core` is centered on systems programming work in R. The technical target is: Build an R toolkit that studies arena behavior through layout fixtures, with stable geometry snapshots and local-only command execution.
...
The score is deterministic. The current threshold is 182. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 6.
